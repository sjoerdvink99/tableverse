...
export function GridCanvas({ width, height }: Props) {
  ...
  useEffect(() => {
    ...
    if (isSatelliteBlending && modeRes.secondary !== null) {
      ...
      renderCells(ctx, width, height, cellW, cellH, viewport.scrollX, viewport.scrollY, tiles, loading, provisionalTiles, viewHash, effectiveRowCount, source.n_cols, virtualSchema, colors, source, allStats, { primary: cellMode, secondary: null, blend: 0 }, isDark, cellAlpha, tileRows, tileCols);
      return;
    }
    ...
    renderCells(
      ...
      tiles, loading, provisionalTiles, viewHash,
      effectiveRowCount, source.n_cols,
      virtualSchema, colors,
      source, allStats, modeRes, isDark, 1.0, tileRows, tileCols,
    );
  }, [width, height, source, viewport, tiles, loading, provisionalTiles, viewHash, zoom, effectiveRowCount, virtualSchema, isDark, semanticZoomEnabled, allStats, rowGroupStats, satelliteEncoding, cellW, cellH]);
  ...
  for (let row = rowStart; row < rowEnd; row++) {
    ...
    for (let col = colStart; col < colEnd; col++) {
      ...
      if (modeRes.secondary === null) {
        renderCellSemantic(ctx, x, y, cellW, cellH, value, colInfo, qStats, fullStats, modeRes.primary, baseAlpha, isDark);
      } else {
        renderCellSemantic(ctx, x, y, cellW, cellH, value, colInfo, qStats, fullStats, modeRes.primary, baseAlpha * (1.0 - modeRes.blend), isDark);
        renderCellSemantic(ctx, x, y, cellW, cellH, value, colInfo, qStats, fullStats, modeRes.secondary, baseAlpha * modeRes.blend, isDark);
      }
    }
