...
impl ExternalSorter {
    ...
    fn do_merge(
        ...
        let mut active: Vec<usize> = Vec::with_capacity(n_runs);
        ...
        while !active.is_empty() {
            let best_pos = active
                ...
                .enumerate()
                .min_by(|&(_, &a), &(_, &b)| {
                    let ca = cursors[a].as_ref().unwrap();
                    let cb = cursors[b].as_ref().unwrap();
                    ca.encoded.row(ca.row).cmp(&cb.encoded.row(cb.row))
                })
                .map(|(pos, _)| pos)
                .unwrap();
            let run_idx = active[best_pos];
